...
- [ ] 4.2 Build the A–Z+0–9 landmark training CSV: merge the chosen sets (srisahithis letters + rayeed045 digits, both already MediaPipe keypoints) + ~20 self-recorded samples per sign; normalize (wrist-origin, scale-invariant). ASL-only (skip BSL/Turkish sets)
